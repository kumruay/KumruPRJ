...
def readNews(item):
    ...
    text_box.insert(tk.INSERT,news)
...
text_box=tk.Text(frame3,width=70,height=5)
text_box.grid(row=0,column=0,padx=25,pady=25)
scroll=tk.Scrollbar(frame3,orient=tk.VERTICAL,command=text_box.yview)
...
def moving_avarage(a,n=50):

    ret=np.cumsum(a,dtyp=float)
    ret[n:]=ret[n:]-ret[:-n]
    return ret[n-1:]/n
...
def update():
    ...
    if method.get()=="m1":
        if item=="EUR/USD":
            ...
            ax3.set_xlin(0,len(data_close_array)+10)
            line3.set_ydata(data_close_array)
            line3.set_xdata(range(len(data_close_array)))

            ax4.set_xline(0,len(data_close_array)+10)
        ...
        if item == "EUR/USD":
            ...
            n = 50
            long_rolling = moving_avarage(data_close_array, n)
